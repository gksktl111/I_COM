...
- 랜딩은 [정부24+](https://plus.gov.kr/)의 검색 중심 정보 구조를 참고해 정책 검색을 첫 행동으로 제공한다. 검색어는 `/policy?q=검색어`로 전달하고 정책 목록에 즉시 적용한다. 검색창 바로 아래에 자주 찾는 정책어를 배치하고, 다음 섹션에는 향후 공개 공지를 위한 빈 상태를 제공한다.
...
- 랜딩과 헤더의 이동 경로가 반복되지 않도록 서비스 바로가기 대신 정책 검색어를 제공한다. 정부24의 브랜드·색상·그래픽을 복제하지 않고 기존 아이콤 색상과 평면 구조를 유지한다.
