...
[루트 디자인 명세](../../DESIGN.md)를 기준으로 관리자 화면을 제외한 `/login`, `/guide`, `/policy`, `/policy/[id]`, `/policy/match`, `/policy/match/test`, `/map`, `/community`를 개편했다. `/`는 별도 랜딩을 제공하지 않고 `/policy/match`로 이동한다.
...
- 별도 랜딩은 사용하지 않는다. 루트 경로는 핵심 기능인 맞춤 정책 찾기로 바로 이동하고, 정책 둘러보기·주변 시설·커뮤니티는 공통 헤더에서 접근한다.
...
- 랜딩과 헤더의 이동 경로가 중복되지 않도록 랜딩을 제거하고 `/`를 `/policy/match`로 연결했다.
