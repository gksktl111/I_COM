...
- 랜딩은 맞춤 탐색 CTA를 첫 화면에 두고 혜택·6개 지원 분야·결과 해석·출처·보조 서비스를 순서대로 안내한다. 사용할 수 있는 라이선스 가족 사진이 아직 없어 우측은 사실 기반 탐색 안내 패널로 구성했으며, 사진 선정은 남은 시각 자산 작업이다.
...
- 랜딩은 핵심 효익과 맞춤 정책 CTA, 직접 이동할 세 경로만 전면에 남겼다. 커뮤니티와 로그인 준비 상태는 동작하지 않는 검색·버튼 대신 현재 가능한 범위를 한 번만 안내한다.
