-- Recognize child-specific support paths; preserve legacy v1/v2 observations.
...
begin
  if coalesce(jsonb_typeof(v_assessment),'null')<>'object'
    or coalesce(v_assessment->>'version','') not in ('policy-relevance-1','policy-relevance-2','policy-relevance-3')
...
begin
  ...
  if coalesce(v_phase,'') not in ('LIST','DETAIL')
    or coalesce(v_relevance->>'version','') not in ('policy-relevance-1','policy-relevance-2','policy-relevance-3')
