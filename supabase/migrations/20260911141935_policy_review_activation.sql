-- Source-bound re-review can activate only a still-pending policy.
...
begin
  if coalesce(jsonb_typeof(v_assessment),'null')<>'object'
    or coalesce(v_assessment->>'version','') not in ('policy-relevance-1','policy-relevance-2','policy-relevance-3','policy-relevance-review-1')
  ...
  if v_assessment->>'version'='policy-relevance-review-1' then
    if p_payload->'reviewOnly' is distinct from 'true'::jsonb
      or v_assessment->>'status'<>'RELATED'
    ...
    if jsonb_array_length(v_assessment->'evidence')=0
      ...
      or exists(select 1 from jsonb_array_elements(v_assessment->'evidence') e
        where e->>'field' not in ('name','target_text','benefit_text')
    ...
    if v_policy.relevance = v_assessment then
      return jsonb_build_object('sourceId',v_policy.source_id,'status','RELATED','replayed',true);
    end if;
  ...
  elsif v_policy.relevance->>'version'='policy-relevance-review-1' then
