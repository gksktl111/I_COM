-- Source-bound review can activate or exclude a still-pending policy.
...
begin
  ...
  if v_assessment->>'version' in ('policy-relevance-review-1','policy-relevance-review-2') then
    if p_payload->'reviewOnly' is distinct from 'true'::jsonb
      or v_assessment->>'status' not in ('RELATED','UNRELATED')
      or (v_assessment->>'version'='policy-relevance-review-1' and v_assessment->>'status'<>'RELATED')
      or (v_assessment->>'status'='UNRELATED' and jsonb_array_length(v_assessment->'categories')<>0)
    ...
    if jsonb_array_length(v_assessment->'evidence')=0
      or (v_assessment->>'version'='policy-relevance-review-2' and not exists(
