-- 여섯 화면 태그의 실제 지원 근거로 활성화하고 세부 조건 검수는 별도로 보존한다.
...
begin
  if coalesce(jsonb_typeof(v_assessment),'null')<>'object'
    or coalesce(v_assessment->>'version','') not in ('policy-relevance-1','policy-relevance-2','policy-relevance-3','policy-relevance-review-1','policy-relevance-review-2','policy-relevance-review-3','policy-relevance-review-4','policy-relevance-review-5')
  ...
  if v_policy.relevance->>'version'='policy-relevance-review-4'
    ...
    then raise exception 'STALE_REVIEW_DECISION'; end if;
  if v_assessment->>'version' in ('policy-relevance-review-1','policy-relevance-review-2','policy-relevance-review-3','policy-relevance-review-4','policy-relevance-review-5') then
    ...
    if (v_assessment->>'version'<>'policy-relevance-review-4' and v_assessment->>'status'<>'REVIEW' and jsonb_array_length(v_assessment->'evidence')=0)
      ...
      or exists(select 1 from jsonb_array_elements(v_assessment->'categories') c
        where (v_assessment->>'version'<>'policy-relevance-review-5' and c#>>'{}' not in ('임신·출산','양육·보육','아동 돌봄','아동 의료·건강','아동 교육','가족 지원','신혼·자녀가구 주거'))
          or (v_assessment->>'version'='policy-relevance-review-5' and c#>>'{}' not in ('임신·출산','양육·보육','돌봄','의료·건강','아동 교육','주거·생활지원')))
    ...
    if v_assessment->>'version'='policy-relevance-review-5' then
      if v_assessment->>'status' not in ('RELATED','REVIEW')
    ...
    if (v_assessment->>'version'='policy-relevance-review-3' and
      ...
      then raise exception 'STALE_REVIEW_DECISION'; end if;
  elsif v_policy.relevance->>'version' in ('policy-relevance-review-1','policy-relevance-review-2','policy-relevance-review-3','policy-relevance-review-4','policy-relevance-review-5') then
